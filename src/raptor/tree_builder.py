...
class TreeBuilder:
    ...
    def __init__(
        ...
        self.summarization_chain = (
            summarization_chain or SummarizationChain()
        )
        ...
        self.summarization_length = (
            summarization_length or settings.summarization_length
        )
    ...
    def create_leaf_nodes(
        ...
        texts = [
            chunk.page_content if isinstance(chunk, Document) else chunk
            for chunk in chunks
        ]
    ...
    def build_tree(
        ...
        logger.info(
            f"Built tree with {tree.total_nodes} total nodes, "
